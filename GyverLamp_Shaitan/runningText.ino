...
bool fillString(const char* text, CRGB letterColor)
{
  ...
  if (millis() - scrollTimer >= map(modes[currentMode].Speed,1,255,200,50))
  {
    scrollTimer = millis();
    FastLED.clear();
    uint8_t i = 0, j = 0;
    while (text[i] != '\0')
    {
      ...
      {
        drawLetter(text[i], offset + j * (LET_WIDTH + SPACE), letterColor, true);
        i++;
        j++;
      }
    }
...
void printTime(uint32_t thisTime, bool onDemand, bool ONflag) // периодический вывод времени бегущей строкой; onDemand - по требованию, вывод текущего времени; иначе - вывод времени по расписанию
{
  ...
  if (!timeSynched && !timeSets)  return;                   // вывод только в режиме WiFi клиента и только, если имя сервера времени разрезолвлено
                                                                      ...
                                                                      // текущего времени суток, настроек дневного/ночного времени и того, включена ли сейчас матрица
{
  ...
  if (espMode != 1U || (!ntpServerAddressResolved && !timeSets) || ONflag)
  {
...
void drawDigit(uint32_t digit, CRGB letterColor, bool transp)
{
 int8_t offset = WIDTH/2-3;         //5
 if (digit>9) offset+=3;            //2 8
 if (digit>99) offset+=3;           //-1 5 11 
 do {
  drawLetter((digit % 10)+48,offset, letterColor, transp);
  digit=digit/10;
  offset-=6;
 } while (digit>0);
}

void drawLetter(uint8_t letter, int8_t offset, CRGB letterColor, bool transp)
{
  ...
  for (uint8_t i = start_pos; i < finish_pos; i++)
  {
    ...
    for (uint8_t j = 0; j < LET_HEIGHT; j++)
    {
      ...
      if (TEXT_DIRECTION)
      {
        ...
        {
          if (transp) drawPixelXY(offset + i, TEXT_HEIGHT + j, 0x000000);
        }
      }
      else
      {
        ...
        {
          if (transp) drawPixelXY(i, offset + TEXT_HEIGHT + j, 0x000000);
        }
      }
    }
  }
...
{
  if (asciiCode<32) //
    return pgm_read_byte(&fontHEX[asciiCode+159][row]);     // Служебные символы в конце таблицы
